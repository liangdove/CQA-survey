...
def evaluate_vqa(dataset_path=None, split='test', output_file='results.json', limit=None, cache_dir="./chartqa_data"):
    ...
    for i, sample in enumerate(dataset):
        try:
            ...
            print(f"Image info: {type(image)} - Size: {image.size if hasattr(image, 'size') else 'Unknown'}")
            print(f"Question: {query}")
            print(f"Correct answers: {labels}")
            ...
            # Print model response and result
            ...
            if (i + 1) % 10 == 0:
                ...
                print(f"\n--- Progress: {i+1}/{len(dataset)}, Current Accuracy: {current_acc:.4f} ({correct}/{total}) ---")
...
if __name__ == "__main__":
    ...
    if args.download_only:
        ...
        evaluate_vqa(
            dataset_path="C:\\E\\CQA-survey\\ChartQA_data\\HuggingFaceM4___chart_qa",
            split=args.split,
            output_file=args.output,
            limit=args.limit,
            cache_dir="C:\\E\\CQA-survey\\ChartQA_data\\HuggingFaceM4___chart_qa"
        )
