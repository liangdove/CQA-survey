...
多模态提示设计的核心策略——结构化输入和格式约束，这对用户的场景特别适用。我可以借鉴其中的"任务指令+模态标记+权重提示+输出要求"框架来设计提示词。

则强调指令必须极其具体，避免模糊表达。这对组织架构图的问答生成很关键，比如要明确指定"答案必须为单个职位名称"这样的约束。

设计能覆盖以下类型的问题：节点识别类（如"CEO的直接下属是谁"）、属性类（如"财务总监属于哪个部门"）、计数类（如"市场部下有几个子部门"）、层级类（如"研发总监处于第几层级"）、路径查找类（如"从产品经理到CEO的汇报路径长度"）等。

采用ChartQA这种单词回答模式比较合适，因为有明确的回答模式，便于后续测评，并且不会像选择题、判断题这么僵硬。

### 构建流程

- 使用Web Scrap 、Chromedriver等策略爬取搜索到的网络图片。
- 为了数据的多样性，使用query搜索引擎，增加爬下来的组织结构图的多样性。
- 得到爬取的图片后，使用最先进的开源多模态大模型（InternVl3-78B），生成对应图像的问答对。
- 为了得到高质量的，适合单词回答模式的问答对，需要精心设计prompt:
```python
...
- 采用批次处理策略，在RPM\TPM周围试探，尽可能加快生成速度。（实测问答对生成耗时1.5h）
- 人工校验问答对的正确性（耗时10h）


### 构建结果

- 生成了**1018**张高质量的类型为组织结构图的问答数据集。
- 由易到难，设计了约300道简单题、400道中等题、300道高难度题。
- 问题格式严格遵守pormpt的约束条件。
